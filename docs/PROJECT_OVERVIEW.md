...
### 1-1. 예문
- 예문은 우리가 미리 준비해 둔 데이터를 **단순 조회**한다 (AI 없음).
- 한 문장은 한국어 + 일본어 + 영어 세트이고, 카테고리로 분류된다.
...
| **CATEGORY** | id, name | 예문과 스토리의 분류 (예: 여행, 식당) |
| **SENTENCE** | id, category_id, jpn_id, eng_id, kor | 한국어 + 일본어 + 영어를 한 세트로 묶는 핵심 테이블 |
...
| 관계 | 의미 |
|---|---|
| JAPANESE / ENGLISH → SENTENCE | 일본어·영어 예문 하나를 여러 문장 행이 참조 |
| CATEGORY → SENTENCE, THEME | 카테고리 하나에 여러 예문과 여러 스토리 |
...
| UNIQUE `(user_id, sentence_id)` | 북마크 중복 방지 | 권장 |
...
- [ ] DB 생성 (schema.sql) 및 UNIQUE 제약 반영
- [ ] 예문 데이터 입력
- [ ] 예문 조회 API
- [ ] 스토리 흐름 (가짜 퀴즈)
- [ ] AI 퀴즈 생성 + 검증 + 저장
